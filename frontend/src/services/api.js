...
export const api = {
  ...
  isThisDeviceTrusted: () => request('GET', `/api/auth/trust-device?device_id=${encodeURIComponent(getDeviceId())}`),
  trustThisDevice: () => request('POST', '/api/auth/trust-device', { device_id: getDeviceId() }),
  untrustThisDevice: () => request('DELETE', `/api/auth/trust-device?device_id=${encodeURIComponent(getDeviceId())}`),
  ...
  getPlaidDetectedFlags: (circleId) => request('GET', `/api/circles/${circleId}/plaid/flags`),
