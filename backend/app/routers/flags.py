...
def get_flags(circle_id: int, is_resolved: Optional[bool] = None, db: Session = Depends(get_db), circle=Depends(require_circle_access)):
...
def create_flag(circle_id: int, body: FlagCreate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db), circle=Depends(require_circle_access)):
...
def update_flag(circle_id: int, flag_id: int, body: FlagUpdate, db: Session = Depends(get_db), circle=Depends(require_circle_access)):
