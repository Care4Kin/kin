...
def get_bills(circle_id: int, is_paid: Optional[bool] = None, db: Session = Depends(get_db), circle=Depends(require_bills_access)):
...
def update_bill(circle_id: int, bill_id: int, body: BillUpdate, db: Session = Depends(get_db), circle=Depends(require_bills_access)):
...
def delete_bill(circle_id: int, bill_id: int, db: Session = Depends(get_db), circle=Depends(require_bills_access)):
    bill = db.query(Bill).filter(Bill.bill_id == bill_id, Bill.circle_id == circle_id).first()
    if not bill:
        raise HTTPException(404, 'Bill not found')
    db.delete(bill)
