...
def get_accounts(circle_id: int, category: Optional[str] = None, db: Session = Depends(get_db), circle=Depends(require_accounts_access)):
...
def create_account(circle_id: int, body: AccountCreate, db: Session = Depends(get_db), circle=Depends(require_accounts_access)):
...
def update_account(circle_id: int, account_id: int, body: AccountUpdate, db: Session = Depends(get_db), circle=Depends(require_accounts_access)):
...
def delete_account(circle_id: int, account_id: int, db: Session = Depends(get_db), circle=Depends(require_accounts_access)):
    account = db.query(Account).filter(Account.account_id == account_id, Account.circle_id == circle_id).first()
    if not account:
        raise HTTPException(404, 'Account not found')
    db.delete(account)
